...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/09 10:31:42 by Juyeong Maing     #+#    #+#             */
/*   Updated: 2022/09/09 13:07:47 by Juyeong Maing    ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_err	even_routine(t_per_philosopher *self)
{
...
static t_err	taken(t_per_philosopher *self)
{
	...
	return (
		...
		|| printf(
			g_taken,
			(size_t)z(sub(philo->started, now)) / 1000,
			self->index)
		< 0
	);
}
...
static t_err	eating(t_per_philosopher *self)
{
	...
	return (
		...
		|| printf(
			g_eating,
			(size_t)z(sub(philo->started, self->last_ate)) / 1000,
			self->index
		) < 0
